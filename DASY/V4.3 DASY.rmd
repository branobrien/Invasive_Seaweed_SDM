...
dasy.max.final.2 <- maxent(x = envi.global.final,
                         p = occur.dasy,
                         path="DASY/Maxent Outputs 4.1/dasy.max.final.2", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
dasy.max.final.3 <- maxent(x = envi.global.final,
                         p = occur.dasy,
                         path="DASY/Maxent Outputs 4.1/dasy.max.final.3", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
dasy.max.final.4 <- maxent(x = envi.global.final,
                         p = occur.dasy,
                         path="DASY/Maxent Outputs 4.1/dasy.max.final.4", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
dasy.max.final.5 <- maxent(x = envi.global.final,
                         p = occur.dasy,
                         path="DASY/Maxent Outputs 4.1/dasy.max.final.5", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
#project model onto global scale
predict.dasy.1 <- predict(dasy.max.final.1, envi.global.final)
predict.dasy.2 <- predict(dasy.max.final.2, envi.global.final)
predict.dasy.3 <- predict(dasy.max.final.3, envi.global.final)
predict.dasy.4 <- predict(dasy.max.final.4, envi.global.final)
predict.dasy.5 <- predict(dasy.max.final.5, envi.global.final)


ggplot() +
  layer_spatial(predict.dasy.1)

#save as raster for plotting in arcmap
writeRaster(predict.dasy.1, filename = 'DASY/Maxent Outputs 4.1/dasy.max.final.1/predict.dasy.1.asc',format='ascii', overwrite = T)
writeRaster(predict.dasy.2, filename = 'DASY/Maxent Outputs 4.1/dasy.max.final.2/predict.dasy.2.asc',format='ascii', overwrite = T)
writeRaster(predict.dasy.3, filename = 'DASY/Maxent Outputs 4.1/dasy.max.final.3/predict.dasy.3.asc',format='ascii', overwrite = T)
writeRaster(predict.dasy.4, filename = 'DASY/Maxent Outputs 4.1/dasy.max.final.4/predict.dasy.4.asc',format='ascii', overwrite = T)
writeRaster(predict.dasy.5, filename = 'DASY/Maxent Outputs 4.1/dasy.max.final.5/predict.dasy.5.asc',format='ascii', overwrite = T)



dasy.final.asc.1 <- raster("DASY/Maxent Outputs 4.1/dasy.max.final.1/predict.dasy.1.asc")
dasy.final.asc.2 <- raster("DASY/Maxent Outputs 4.1/dasy.max.final.2/predict.dasy.2.asc")
dasy.final.asc.3 <- raster("DASY/Maxent Outputs 4.1/dasy.max.final.3/predict.dasy.3.asc")
dasy.final.asc.4 <- raster("DASY/Maxent Outputs 4.1/dasy.max.final.4/predict.dasy.4.asc")
dasy.final.asc.5 <- raster("DASY/Maxent Outputs 4.1/dasy.max.final.5/predict.dasy.5.asc")


ggplot() +
  layer_spatial(dasy.final.asc.1)

### Now that we have the outputs in .asc format, need to average them all together into one raster

dasy.final.stack <- stack(c(
  dasy.final.asc.1,
  dasy.final.asc.2,
  dasy.final.asc.3,
  dasy.final.asc.4,
...
dasy.final.avg.asc <- calc(dasy.final.stack, fun = mean)


### then cut the raster based on equal TSS
### using the average equal TSS from all 5 iterations
### equal TSS = ___0.4634____

dasy.final.thresh <- dasy.final.avg.asc

dasy.final.thresh[dasy.final.thresh < 0.4634 ] <- NA # threshold value, change to proper number (above)

writeRaster(dasy.final.thresh, filename = 'ArcMap/DASY Arc 4.3/dasy.final.thresh.asc',format='ascii', overwrite = T)
...
dasy.RCP45.1 <- predict(dasy.max.final.1, envi.global.RCP45)
dasy.RCP45.2 <- predict(dasy.max.final.2, envi.global.RCP45)
dasy.RCP45.3 <- predict(dasy.max.final.3, envi.global.RCP45)
dasy.RCP45.4 <- predict(dasy.max.final.4, envi.global.RCP45)
dasy.RCP45.5 <- predict(dasy.max.final.5, envi.global.RCP45)

dasy.RCP45.stack <- stack(c(dasy.RCP45.1,
  dasy.RCP45.2,
  dasy.RCP45.3,
  dasy.RCP45.4,
  dasy.RCP45.5))

dasy.RCP45.avg <- calc(dasy.RCP45.stack, fun = mean)


writeRaster(dasy.RCP45.avg, filename = "ArcMap/dasy Arc 4.3/dasy.RCP45.avg.asc", format = "ascii", overwrite = T)


## need to cut to threshold again

dasy.RCP45.thresh <- dasy.RCP45.avg

dasy.RCP45.thresh[dasy.RCP45.thresh < 0.4634] <- NA

writeRaster(dasy.RCP45.thresh, filename = "ArcMap/dasy Arc 4.3/dasy.RCP45.avg.thresh.asc", format = "ascii", overwrite = T)
