...
predict.codi.max <- raster("CODI/Maxent Outputs 4.1 GUI/codi.max.final/codium_fragile_avg.asc")

plot(predict.codi.max)


#ok, got that, next need to cut the layer based on threshold found via maxent
# equa test sensitivity and specifity = 0.3936

predict.codi.thresh <- predict.codi.max
predict.codi.thresh[predict.codi.thresh < 0.3936] <- NA # threshold value, change to proper number (above)
...
writeRaster(predict.codi.thresh,
            filename = 'ArcMap/CODI Arc 4.1/predict.codi.thresh.asc',
            format = 'ascii', 
            overwrite = T)
...
predict.codi.rcp45 <- raster("CODI/Maxent Outputs 4.1 GUI/codi.max.future/Codium_fragile_fixed_avg.asc")
...
predict.codi.rcp45.thresh <- predict.codi.rcp45
predict.codi.rcp45.thresh[predict.codi.rcp45.thresh < 0.3936] <- NA 

writeRaster(predict.codi.rcp45.thresh,
            filename = 'ArcMap/CODI Arc 4.1/predict.codi.rcp45.thresh.asc',
            format = 'ascii', 
            overwrite = T)
...
```{r sidebar}
## errors with things not having the same geographic coordinates...
#### 

## read in sst.max.future raster
## check its extent
...
writeRaster(SST.max.RCP45, filename = 'Environmental Data/RCP45/fixed/e08.SST.max.asc', format = 'ascii', overwrite = T)
writeRaster(SST.min.RCP45, filename = 'Environmental Data/RCP45/fixed/e09.SST.min.asc', format = 'ascii', overwrite = T)

# check PHO max extent
PHO.max.cc <- raster('Environmental Data/RCP45/e05.PHO.max.asc')
PHO.max.cc
# it's 64.999, not 65. That's the problem there

# try to fix by aligning pho onto sst extent

PHO.max.cc <- resample(PHO.max.cc, SST.max.RCP45, 'bilinear')

PHO.max.cc  #seems like it worked

# now export that file back out...

writeRaster(PHO.max.cc, filename = 'Environmental Data/RCP45/fixed/e05.PHO.max.asc', format = 'ascii', overwrite = T)
