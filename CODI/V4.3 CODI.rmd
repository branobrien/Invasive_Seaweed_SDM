...
writeRaster(predict.codi.global, 
            filename = 'CODI/predict.codi.global.full.asc',
            format='ascii')
...
writeRaster(predict.codi.thresh,
            filename = 'CODI/predict.codi.thresh.asc',
            formatt= 'ascii')
...
## May 16, 2022
# marking new version 4.3, which is basically same as 4.1, except using 5 iterations of the final model and anveraging them
# keeping all the orig 4.1 folders and stuff though
```
...
codi.max.final.2 <- maxent(x = envi.global.final,
                         p = occur.codi,
                         path="CODI/Maxent Outputs 4.1/codi.max.final.2", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
codi.max.final.3 <- maxent(x = envi.global.final,
                         p = occur.codi,
                         path="CODI/Maxent Outputs 4.1/codi.max.final.3", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
codi.max.final.4 <- maxent(x = envi.global.final,
                         p = occur.codi,
                         path="CODI/Maxent Outputs 4.1/codi.max.final.4", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
codi.max.final.5 <- maxent(x = envi.global.final,
                         p = occur.codi,
                         path="CODI/Maxent Outputs 4.1/codi.max.final.5", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
#project model onto global scale
predict.codi.1 <- predict(codi.max.final.1, envi.global.final)
predict.codi.2 <- predict(codi.max.final.2, envi.global.final)
predict.codi.3 <- predict(codi.max.final.3, envi.global.final)
predict.codi.4 <- predict(codi.max.final.4, envi.global.final)
predict.codi.5 <- predict(codi.max.final.5, envi.global.final)


ggplot() +
  layer_spatial(predict.codi.1)

#save as raster for plotting in arcmap
writeRaster(predict.codi.1, filename = 'CODI/Maxent Outputs 4.1/codi.max.final.1/predict.codi.1.asc',format='ascii', overwrite = T)
writeRaster(predict.codi.2, filename = 'CODI/Maxent Outputs 4.1/codi.max.final.2/predict.codi.2.asc',format='ascii', overwrite = T)
writeRaster(predict.codi.3, filename = 'CODI/Maxent Outputs 4.1/codi.max.final.3/predict.codi.3.asc',format='ascii', overwrite = T)
writeRaster(predict.codi.4, filename = 'CODI/Maxent Outputs 4.1/codi.max.final.4/predict.codi.4.asc',format='ascii', overwrite = T)
writeRaster(predict.codi.5, filename = 'CODI/Maxent Outputs 4.1/codi.max.final.5/predict.codi.5.asc',format='ascii', overwrite = T)



codi.final.asc.1 <- raster("CODI/Maxent Outputs 4.1/codi.max.final.1/predict.codi.1.asc")
codi.final.asc.2 <- raster("CODI/Maxent Outputs 4.1/codi.max.final.2/predict.codi.2.asc")
codi.final.asc.3 <- raster("CODI/Maxent Outputs 4.1/codi.max.final.3/predict.codi.3.asc")
codi.final.asc.4 <- raster("CODI/Maxent Outputs 4.1/codi.max.final.4/predict.codi.4.asc")
codi.final.asc.5 <- raster("CODI/Maxent Outputs 4.1/codi.max.final.5/predict.codi.5.asc")


ggplot() +
  layer_spatial(codi.final.asc.1)

### Now that we have the outputs in .asc format, need to average them all together into one raster

codi.final.stack <- stack(c(
  codi.final.asc.1,
  codi.final.asc.2,
  codi.final.asc.3,
  codi.final.asc.4,
...
codi.final.avg.asc <- calc(codi.final.stack, fun = mean)


### then cut the raster based on equal TSS
### using the average equal TSS from all 5 iterations
### equal TSS = ___0.5438____

codi.final.thresh <- codi.final.avg.asc

codi.final.thresh[codi.final.thresh < 0.5438 ] <- NA # threshold value, change to proper number (above)

writeRaster(codi.final.thresh, filename = 'ArcMap/CODI Arc 4.3/codi.final.thresh.asc',format='ascii', overwrite = T)
...
codi.RCP45.1 <- predict(codi.max.final.1, envi.global.RCP45)
codi.RCP45.2 <- predict(codi.max.final.2, envi.global.RCP45)
codi.RCP45.3 <- predict(codi.max.final.3, envi.global.RCP45)
codi.RCP45.4 <- predict(codi.max.final.4, envi.global.RCP45)
codi.RCP45.5 <- predict(codi.max.final.5, envi.global.RCP45)

codi.RCP45.stack <- stack(c(codi.RCP45.1,
  codi.RCP45.2,
  codi.RCP45.3,
  codi.RCP45.4,
  codi.RCP45.5))

codi.RCP45.avg <- calc(codi.RCP45.stack, fun = mean)


writeRaster(codi.RCP45.avg, filename = "ArcMap/CODI Arc 4.3/codi.RCP45.avg.asc", format = "ascii", overwrite = T)


## need to cut to threshold again

codi.RCP45.thresh <- codi.RCP45.avg

codi.RCP45.thresh[codi.RCP45.thresh < 0.5438] <- NA

writeRaster(codi.RCP45.thresh, filename = "ArcMap/CODI Arc 4.3/codi.RCP45.avg.thresh.asc", format = "ascii", overwrite = T)
