...
predict.sarg.global <- predict(sarg.max.final, envi.global.final)

ggplot() +
  layer_spatial(predict.sarg.global)

#save as raster for plotting in arcmap
writeRaster(predict.sarg.global, 
            filename = 'SARG/predict.sarg.global.full.asc',
            format='ascii')
...
predict.sarg.thresh <- predict.sarg.global
predict.sarg.thresh[predict.sarg.thresh < 0.284] <- NA # this value changes by species

ggplot() +
  layer_spatial(predict.sarg.thresh)

writeRaster(predict.sarg.thresh,
            filename = 'SARG/predict.sarg.thresh.asc',
            formatt= 'ascii')
...
sarg.occur.shp <- read_sf("ArcMap/SARG Arc/SARG R/SARG_occur.shp")
sarg.vuln.shp <- read_sf("ArcMap/SARG Arc/SARG R/SARG_vuln.shp")
  

ggplot() +
  geom_sf(data=world, color='black', fill='gray') + 
  geom_sf(data = sarg.occur.shp, color = 'black', fill = 'black') +
  geom_sf(data = sarg.vuln.shp, color = 'black', fill = 'red') +
  coord_sf(expand = F) +
  theme_map
...
sarg.high.cc.shp <- read_sf("ArcMap/SARG Arc/SARG R/SARG_high_cc.shp")
sarg.gain.shp <- read_sf("ArcMap/SARG Arc/SARG R/SARG_gain.shp")
sarg.loss.shp <- read_sf("ArcMap/SARG Arc/SARG R/SARG_loss.shp")

  
ggplot() +
  geom_sf(data=world, color='black', fill='gray') + 
  geom_sf(data = sarg.high.cc.shp, color = 'black', fill = 'orange') +
  geom_sf(data = sarg.gain.shp, color= 'black', fill = 'green') +
  geom_sf(data = sarg.loss.shp, color = 'black', fill = 'purple') +
  coord_sf(expand = F) +
  theme_map
