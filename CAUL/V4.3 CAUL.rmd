...
caul.max.final.3 <- maxent(x = envi.global.final,
                         p = occur,
                         path="CAUL/Maxent Outputs 4.1/caul.max.final.3", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
caul.max.final.4 <- maxent(x = envi.global.final,
                         p = occur,
                         path="CAUL/Maxent Outputs 4.1/caul.max.final.4", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
caul.max.final.5 <- maxent(x = envi.global.final,
                         p = occur,
                         path="CAUL/Maxent Outputs 4.1/caul.max.final.5", 
                         args=c('linear=TRUE',       
                                'quadratic=TRUE',     
                                'product=FALSE', 
                                'threshold=FALSE', 
...
#project model onto global scale
predict.caul.1 <- predict(caul.max.final.1, envi.global.final)
predict.caul.2 <- predict(caul.max.final.2, envi.global.final)
predict.caul.3 <- predict(caul.max.final.3, envi.global.final)
predict.caul.4 <- predict(caul.max.final.4, envi.global.final)
predict.caul.5 <- predict(caul.max.final.5, envi.global.final)


#save as raster for plotting in arcmap
writeRaster(predict.caul.1, filename = 'CAUL/Maxent Outputs 4.1/caul.max.final.1/predict.caul.1.asc',format='ascii', overwrite = T)
writeRaster(predict.caul.2, filename = 'CAUL/Maxent Outputs 4.1/caul.max.final.2/predict.caul.2.asc',format='ascii', overwrite = T)
writeRaster(predict.caul.3, filename = 'CAUL/Maxent Outputs 4.1/caul.max.final.3/predict.caul.3.asc',format='ascii', overwrite = T)
writeRaster(predict.caul.4, filename = 'CAUL/Maxent Outputs 4.1/caul.max.final.4/predict.caul.4.asc',format='ascii', overwrite = T)
writeRaster(predict.caul.5, filename = 'CAUL/Maxent Outputs 4.1/caul.max.final.5/predict.caul.5.asc',format='ascii', overwrite = T)



caul.final.asc.1 <- raster("CAUL/Maxent Outputs 4.1/caul.max.final.1/predict.caul.1.asc")
caul.final.asc.2 <- raster("CAUL/Maxent Outputs 4.1/caul.max.final.2/predict.caul.2.asc")
caul.final.asc.3 <- raster("CAUL/Maxent Outputs 4.1/caul.max.final.3/predict.caul.3.asc")
caul.final.asc.4 <- raster("CAUL/Maxent Outputs 4.1/caul.max.final.4/predict.caul.4.asc")
caul.final.asc.5 <- raster("CAUL/Maxent Outputs 4.1/caul.max.final.5/predict.caul.5.asc")


### Now that we have the outputs in .asc format, need to average them all together into one raster

caul.final.stack <- stack(c(
  caul.final.asc.1,
  caul.final.asc.2,
  caul.final.asc.3,
  caul.final.asc.4,
...
caul.final.avg.asc <- calc(caul.final.stack, fun = mean)
...
caul.final.thresh <- caul.final.avg.asc

caul.final.thresh[caul.final.thresh < 0.549] <- NA # threshold value, change to proper number (above)

writeRaster(caul.final.thresh, filename = 'ArcMap/CAUL Arc 4.3/caul.final.thresh.asc',format='ascii', overwrite = T)
...
caul.RCP45.1 <- predict(caul.max.final.1, envi.global.RCP45)
caul.RCP45.2 <- predict(caul.max.final.2, envi.global.RCP45)
caul.RCP45.3 <- predict(caul.max.final.3, envi.global.RCP45)
caul.RCP45.4 <- predict(caul.max.final.4, envi.global.RCP45)
caul.RCP45.5 <- predict(caul.max.final.5, envi.global.RCP45)


caul.RCP45.stack <- stack(c(
  caul.RCP45.1,
  caul.RCP45.2,
  caul.RCP45.3,
  caul.RCP45.4,
...
caul.RCP45.avg <- calc(caul.RCP45.stack, fun = mean)


writeRaster(caul.RCP45.avg, filename = "ArcMap/CAUL Arc 4.3/caul.RCP45.avg.asc", format = "ascii", overwrite = T)


## need to cut to threshold again

caul.RCP45.thresh <- caul.RCP45.avg

caul.RCP45.thresh[caul.RCP45.thresh < 0.549] <- NA

writeRaster(caul.RCP45.thresh, filename = "ArcMap/CAUL Arc 4.3/caul.RCP45.avg.thresh.asc", format = "ascii", overwrite = T)
